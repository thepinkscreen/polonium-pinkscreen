...
public sealed partial class HitscanBasicVisualsSystem : EntitySystem
{
    ...
    private void FireEffects(EntityCoordinates fromCoordinates, float distance, Angle shotAngle, HitscanBasicVisualsComponent hitscan)
    {
        ...
        // Client cannot RaiseNetworkEvent; legacy hitscan uses the same server-only path in SharedGunSystem.FireEffects.
        if (_netManager.IsServer)
        {
            RaiseNetworkEvent(ev, Filter.Pvs(fromCoordinates, entityMan: EntityManager));
        }
        else
        {
            // Gun prediction runs hitscan on the client — play beam effects locally (see GunSystem.OnHitscan).
            RaiseLocalEvent(ev);
        }
    }
}
